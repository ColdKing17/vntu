<form wire:submit="save">
    <div class="grid max-w-2xl grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
        ...
        <div class="sm:col-span-3">
            <x-label required>Кількість населення</x-label>
            <x-input wire:model="people_amount" class="block mt-1 w-full" type="number" min="0" required />
        </div>

        <div class="sm:col-span-3">
            <x-label required>Кількість терміналів</x-label>
            <x-input wire:model="terminals_amount" class="block mt-1 w-full" type="number" min="0" required />
        </div>
